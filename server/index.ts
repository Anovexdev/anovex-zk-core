...
// CRITICAL: Trust Replit's proxy for secure cookies in production
if (process.env.NODE_ENV === 'production') {
  app.set('trust proxy', 1); // Trust first proxy (Replit's HTTPS termination)
}
...
app.use(session({
  ...
  cookie: {
    ...
    httpOnly: true,
    // CRITICAL FIX: Force secure: true in production (Replit always uses HTTPS proxy)
  ...
  // CRITICAL: Trust first proxy (Replit's HTTPS proxy)
